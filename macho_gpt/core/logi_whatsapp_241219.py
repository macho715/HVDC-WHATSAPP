...
class WhatsAppProcessor:
    ...
    def extract_summary_data(self, messages: List[WhatsAppMessage]) -> Dict:
        ...
        return {
            'status': 'SUCCESS' if confidence >= self.confidence_threshold else 'PARTIAL',
    ...
    def _calculate_confidence(self, messages: List[WhatsAppMessage]) -> float:
        ...
        # 타임스탬프가 있는 메시지 비율
        valid_timestamp_ratio = len(messages) / len(messages)  # 모든 메시지가 파싱되었다면 1.0
        
        # 발신자 정보가 있는 메시지 비율
        valid_sender_ratio = len([msg for msg in messages if msg.sender]) / len(messages)
        
        # 내용이 있는 메시지 비율
        valid_content_ratio = len([msg for msg in messages if msg.content.strip()]) / len(messages)
    ...
    def generate_kpi_summary(self, messages: List[WhatsAppMessage]) -> Dict:
        ...
        return {
            ...
            'urgent_ratio': round(urgent_count / total_messages, 2) if total_messages > 0 else 0,
            'important_ratio': round(important_count / total_messages, 2) if total_messages > 0 else 0,
